...
function updateRocket(dt, $container, x) {
    ...
    if (WINDOW_STATE.rocketX != x && x != null){
      if (click == true){
        WINDOW_STATE.rocketX = x;
        click = false;
      }
    }
...
function createHeader($container, $element,name) {
  ...
  function update(e) {
    ...
    experience.addEventListener("click", (event) => {
      getElementX(experience)
      click = true
      updateRocket(dt, $container, x)
    })


    const more = document.getElementById('about')
    about.addEventListener("click", (event) => {
      getElementX(about)
      click = true
      updateRocket(dt, $container, x)
