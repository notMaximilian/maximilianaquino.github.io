...
function openTab ($container, $element){
  ...
  for (let j = 0; j < headers.length; j++) {
    ...
    if (header.name != $element.name){
      header.$element.classList.add('fade')
    }
  }
  ...
  destroyAll($container, $element.name)
  $element.classList.add('open-tab')
}
...
function updateRocket(dt, $container) {
  ...
  function destroyAll($container, exception){
    ...
    for (let x = 0; x < headers.length; x++){
      let head = headers[x];
      if (head.dead == true || head.name == exception){continue}
      destroyHeader(head)      
    }
    const rocket = document.querySelector('.rocket')
      ...
      for (let j = 0; j < headers.length; j++) {
        ...
        if (rectsIntersect(r1, r2)) {
          if(header.name == 'about'){
            openTab($container, header.$element)
            destroyLaser($container, laser);
